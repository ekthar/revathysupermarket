...
function getUploadClient() {
  // Prefer R2 if configured, fall back to S3
  if (process.env.CLOUDFLARE_R2_ACCOUNT_ID) {
    return {
      ...
      bucket: process.env.CLOUDFLARE_R2_BUCKET || "store-uploads",
      getUrl: (key: string) => getR2PublicUrl(key)
    };
  }
  ...
  return {
    ...
    bucket: process.env.S3_BUCKET || "store-uploads",
    getUrl: (key: string) => `https://${process.env.S3_BUCKET || "store-uploads"}.s3.${process.env.AWS_REGION || "ap-south-1"}.amazonaws.com/${key}?v=${Date.now()}`
  };
}

// POST /api/admin/logo - Upload store logo OR save logo URL
export async function POST(request: Request) {
  ...
  try {
    const { client, bucket, getUrl } = getUploadClient();
    ...
    return NextResponse.json({ error: "Upload failed. Try pasting an image URL instead." }, { status: 500 });
  }
}
