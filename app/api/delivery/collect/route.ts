...
export async function POST(request: Request) {
  ...
  const limit = await enforceRateLimit(`delivery:collect:${session.user.id}`, 30, 300);
  ...
  const order = await prisma.order.findFirst({ where: { id: parsed.data.orderId, deliveryPartnerId: session.user.id, status: { in: ["OUT_FOR_DELIVERY", "ARRIVING"] } }, select: { id: true, total: true } });
  ...
  const collection = await prisma.deliveryCollection.upsert({ where: { orderId: order.id }, create: { orderId: order.id, partnerId: session.user.id, expectedAmount: remainingPayable, cashCollected: parsed.data.cashCollected, upiCollected: parsed.data.upiCollected, walletApplied, adjustmentAmount, upiReference: parsed.data.upiReference ?? null, status }, update: { expectedAmount: remainingPayable, cashCollected: parsed.data.cashCollected, upiCollected: parsed.data.upiCollected, walletApplied, adjustmentAmount, upiReference: parsed.data.upiReference ?? null, status, discrepancyReason: null } });
