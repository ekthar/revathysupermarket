...
export async function POST(request: Request) {
  ...
  const limit = await enforceRateLimit(`delivery:damage:${authResult.userId}`, 20, 3600);
  ...
  const order = await prisma.order.findFirst({ where: { id: orderId, deliveryPartnerId: authResult.userId, status: { in: ["OUT_FOR_DELIVERY", "ARRIVING"] } }, include: { items: true, deliveryAdjustments: { where: { status: { in: ["APPROVED", "PENDING_APPROVAL"] } } } } });
  ...
  const adjustment = await prisma.deliveryAdjustment.create({ data: { orderId, orderItemId, partnerId: authResult.userId, itemName: item.name, quantity, reason, reductionAmount, evidenceUrl: evidenceUrl ?? null, status: needsApproval ? "PENDING_APPROVAL" : "APPROVED", approvedAt: needsApproval ? null : new Date() } });
