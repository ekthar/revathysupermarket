  ...
  return (
    ...
    >
      ...
      <View className="flex-row flex-wrap gap-3">
        <MetricCard label="Orders Today" value={stats?.todayOrders ?? 0} color="bg-blue-50 dark:bg-blue-950/30" textColor="text-blue-700 dark:text-blue-300" />
        <MetricCard label="Revenue Today" value={`₹${(stats?.todayRevenue ?? 0).toFixed(2)}`} color="bg-emerald-50 dark:bg-emerald-950/30" textColor="text-emerald-700 dark:text-emerald-300" />
        <MetricCard label="Pending" value={stats?.pendingOrders ?? 0} color="bg-amber-50 dark:bg-amber-950/30" textColor="text-amber-700 dark:text-amber-300" />
        <MetricCard label="Delivered" value={stats?.deliveredOrders ?? 0} color="bg-purple-50 dark:bg-purple-950/30" textColor="text-purple-700 dark:text-purple-300" />
      </View>
...
function MetricCard({ label, value, color, textColor }: { label: string; value: number | string; color: string; textColor: string }) {
  return (
    <View
      className={`flex-1 min-w-[45%] rounded-2xl p-4 border border-slate-100 dark:border-slate-800 ${color}`}
