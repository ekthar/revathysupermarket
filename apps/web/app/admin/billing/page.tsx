  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-emerald-50 to-lime-50 dark:from-emerald-950/20 dark:to-slate-900 p-5">
        ...
        <h1 className="mt-1 text-xl font-bold text-foreground">Billing Report</h1>
        <p className="text-caption text-muted-foreground mt-1">Revenue from delivered orders only</p>
      </div>
      ...
      <div className="rounded-2xl bg-card border border-border p-4">
        <h2 className="text-body font-bold text-foreground mb-3">Payment Method Breakdown</h2>
      ...
      <div className="rounded-2xl bg-card border border-border overflow-hidden">
        <div className="flex items-center justify-between px-4 pt-4 pb-2">
          <h2 className="text-body font-bold text-foreground">Recent Delivered</h2>
        ...
        <div className="divide-y divide-border">
          {recentDelivered.map((order) => (
            <div key={order.orderNumber} className="flex items-center justify-between px-4 py-3">
              <div>
                <p className="text-caption font-semibold text-foreground">#{order.orderNumber}</p>
                <p className="text-micro text-muted-foreground">{order.customerName} &middot; {order.paymentMethod}</p>
              </div>
              <div className="text-right">
                <p className="text-body font-bold text-foreground">{formatCurrency(Number(order.total))}</p>
...
function BillCard({ label, orders, revenue }: { label: string; orders: number; revenue: number }) {
  return (
    <div className="rounded-2xl bg-card border border-border p-4">
      ...
      <p className="text-heading font-bold text-foreground mt-1">{formatCurrency(revenue)}</p>
      <p className="text-micro text-muted-foreground mt-0.5">{orders} orders</p>
    </div>
  );
}
...
function PaymentRow({ label, amount, total, color }: { label: string; amount: number; total: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-caption font-medium text-muted-foreground w-32 shrink-0">{label}</span>
      <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden">
        <div className={`h-full rounded-full ${color}`} style={{ width: `${pct}%` }} />
      </div>
      <span className="text-caption font-bold text-muted-foreground w-16 text-right">{formatCurrency(amount)}</span>
      <span className="text-micro text-muted-foreground w-8 text-right">{pct}%</span>
    </div>
  );
}
