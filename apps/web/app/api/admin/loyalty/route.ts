...
const schema = z.object({ earnRupeesPerPoint: z.coerce.number().min(1).max(1000), pointValueRupees: z.coerce.number().min(0.01).max(10), maxRedemptionPercent: z.coerce.number().min(1).max(100), referralRewardPoints: z.coerce.number().int().min(0).max(10000) });
