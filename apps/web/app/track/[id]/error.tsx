...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="flex min-h-[50vh] flex-col items-center justify-center px-4 text-center">
      ...
      <h2 className="text-xl font-black text-neutral-900 dark:text-white">Something went wrong</h2>
      <p className="mt-2 text-sm text-neutral-500 dark:text-neutral-400 max-w-sm">
        {error.message || "Failed to load tracking information. Please try again."}
      </p>
      <button
        onClick={reset}
