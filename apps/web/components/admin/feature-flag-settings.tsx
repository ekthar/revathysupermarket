    ...
    <div className="space-y-8">
      {FLAG_CATEGORIES.map((category) => (
        <div key={category.title} className="space-y-3">
          <h2 className="text-lg font-black text-foreground px-1">{category.title}</h2>
      ...
      return (
        <label key={cfgKey} className="block">
          ...
          <select
            ...
            className="mt-1 h-9 w-full rounded-lg border border-border bg-muted px-3 text-sm"
          >
      ...
      return (
        <label key={cfgKey} className="block">
          ...
          <div className="mt-1 flex items-center gap-1">
            {hint.prefix && <span className="text-sm font-semibold text-muted-foreground">{hint.prefix}</span>}
            <input
              type="number"
            ...
            />
            {hint.suffix && <span className="text-sm font-semibold text-muted-foreground whitespace-nowrap">{hint.suffix}</span>}
          </div>
        </label>
      );
    }
          ...
          <div className="grid grid-cols-2 gap-3">
            {hints
              ...
              : Object.entries(config).map(([cfgKey, cfgValue]) => (
                  <label key={cfgKey} className="block">
                    <span className="text-xs font-bold text-muted-foreground capitalize">{cfgKey.replace(/([A-Z])/g, " $1")}</span>
                    <input
    ...
    <div className="rounded-2xl border border-border bg-card overflow-hidden shadow-elevation-1">
      <div className="p-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-black text-foreground">Forced Accept Delivery</h3>
            <p className="text-xs text-muted-foreground mt-0.5">Global toggle + per-staff overrides</p>
          </div>
        </div>
        <label className="relative inline-flex items-center cursor-pointer">
      ...
      <div className="border-t border-border px-5 py-4">
        <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">Per-Staff Override</p>
        ...
        <div className="space-y-2 max-h-60 overflow-y-auto">
          {deliveryPartners.map((partner) => (
            <div key={partner.id} className="flex items-center justify-between py-2 px-3 rounded-xl bg-muted">
          ...
          {deliveryPartners.length === 0 && <p className="text-xs text-muted-foreground italic">No delivery partners found</p>}
        </div>
      </div>
    </div>
  );
    ...
    <div className="rounded-2xl border border-border bg-card overflow-hidden shadow-elevation-1">
      <div className="p-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-black text-foreground">Ring Alert Rules</h3>
            <p className="text-xs text-muted-foreground mt-0.5">Ringtone, duration, and escalation for incoming orders</p>
          </div>
        </div>
        <label className="relative inline-flex items-center cursor-pointer">
      ...
      <div className="border-t border-border px-5 py-4 grid grid-cols-2 gap-3">
        <label className="block">
          <span className="text-xs font-bold text-muted-foreground">Ringtone</span>
          <input type="text" defaultValue={ringConfig?.ringtone ?? "default"} onBlur={(e) => { updateFlag("ring_alert_rules", { config: { ...ringConfig, ringtone: e.target.value.trim() || "default" } }); }} className="mt-1 h-9 w-full rounded-lg border border-border bg-muted px-3 text-sm" />
        </label>
        <label className="block">
          <span className="text-xs font-bold text-muted-foreground">Duration (seconds)</span>
          <input type="number" min={5} max={120} defaultValue={ringConfig?.durationSeconds ?? 30} onBlur={(e) => { updateFlag("ring_alert_rules", { config: { ...ringConfig, durationSeconds: Math.max(5, Math.min(120, Number(e.target.value) || 30)) } }); }} className="mt-1 h-9 w-full rounded-lg border border-border bg-muted px-3 text-sm" />
        </label>
        <label className="block">
          <span className="text-xs font-bold text-muted-foreground">Escalation After (seconds)</span>
          <input type="number" min={10} max={300} defaultValue={ringConfig?.escalationAfterSeconds ?? 60} onBlur={(e) => { updateFlag("ring_alert_rules", { config: { ...ringConfig, escalationAfterSeconds: Math.max(10, Math.min(300, Number(e.target.value) || 60)) } }); }} className="mt-1 h-9 w-full rounded-lg border border-border bg-muted px-3 text-sm" />
        </label>
        <label className="block">
          <span className="text-xs font-bold text-muted-foreground">Escalation Target</span>
          <select defaultValue={ringConfig?.escalationTarget ?? "admin"} onChange={(e) => { updateFlag("ring_alert_rules", { config: { ...ringConfig, escalationTarget: e.target.value } }); }} className="mt-1 h-9 w-full rounded-lg border border-border bg-muted px-3 text-sm">
    ...
    <div className="rounded-2xl border border-border bg-card overflow-hidden shadow-elevation-1">
      <div className="p-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-black text-foreground">Print Required Alert</h3>
            <p className="text-xs text-muted-foreground mt-0.5">Alert when orders are unprinted past a time threshold</p>
          </div>
        </div>
        <label className="relative inline-flex items-center cursor-pointer">
      ...
      <div className="border-t border-border px-5 py-4">
        <label className="block max-w-xs">
          <span className="text-xs font-bold text-muted-foreground">Threshold (minutes)</span>
          <input type="number" min={1} max={120} defaultValue={printConfig?.thresholdMinutes ?? 10} onBlur={(e) => { updateFlag("print_required_alert", { config: { thresholdMinutes: Math.max(1, Math.min(120, Number(e.target.value) || 10)) } }); }} className="mt-1 h-9 w-full rounded-lg border border-border bg-muted px-3 text-sm" />
          <p className="mt-1 text-xs text-muted-foreground">Orders unprinted longer than this will trigger a dashboard alert</p>
        </label>
      </div>
    </div>
  );
