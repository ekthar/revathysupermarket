...
function ReturnRequestSheet({ order, onClose }: { order: CustomerOrder; onClose: () => void }) {
  ...
  return (
    <BottomSheet open onClose={onClose} title="Request a return">
      ...
      <div className="mt-4 divide-y divide-border rounded-2xl border border-border">
        {order.items.map((item) => (
          <div key={item.id} className="flex items-center justify-between gap-3 p-3">
            ...
            <select aria-label={`Return quantity for ${item.name}`} value={quantities[item.id] ?? 0} onChange={(e) => setQuantities((current) => ({ ...current, [item.id]: Number(e.target.value) }))} className="h-10 rounded-xl border border-border bg-background px-3">
              {Array.from({ length: item.quantity + 1 }, (_, value) => <option key={value} value={value}>{value}</option>)}
            </select>
          </div>
        ))}
      </div>
      <label className="mt-4 block text-sm font-bold">Bill Number / Order Invoice Number
        <input type="text" value={billNumber} onChange={(e) => setBillNumber(e.target.value)} required placeholder="Enter bill number" className="mt-2 h-11 w-full rounded-xl border border-border bg-background px-3" />
      </label>
      <label className="mt-4 block text-sm font-bold">Reason
        <select value={reason} onChange={(e) => setReason(e.target.value)} className="mt-2 h-11 w-full rounded-xl border border-border bg-background px-3">
      ...
      <label className="mt-4 block text-sm font-bold">Tell us what happened
        <textarea value={note} onChange={(e) => setNote(e.target.value)} maxLength={500} className="mt-2 min-h-24 w-full rounded-xl border border-border bg-background p-3" />
      </label>
      <label className="mt-4 block text-sm font-bold">Evidence photo (optional)
        <input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => upload(e.target.files?.[0])} className="mt-2 block w-full text-sm" />
      </label>
      {photoUrl && <p className="mt-2 text-xs font-bold text-secondary-700">Photo uploaded</p>}
