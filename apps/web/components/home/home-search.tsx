...
export function HomeSearch({ suggestions = [] }: { suggestions?: string[] }) {
  ...
  return (
    <div className="sticky top-[var(--mobile-header-height,56px)] md:top-[70px] z-sticky bg-background/95 backdrop-blur-md px-4 py-3">
      <div className="relative mx-auto flex max-w-2xl items-center gap-2.5">
        <button
          ...
          // The visible label rotates, so the accessible name comes from the
          // translated placeholder and stays stable.
        ...
        >
          ...
          {activeTerm ? (
            /* aria-hidden: the animating term would otherwise be announced on
               every rotation. Rendering the bare quoted term (rather than an
               English "Search for …" prefix) keeps this locale-agnostic. */
            <span aria-hidden="true" className="relative min-w-0 flex-1">
              <AnimatePresence mode="wait" initial={false}>
                <motion.span
                  ...
                  className="block truncate text-body font-semibold text-neutral-600 dark:text-neutral-300"
                >
