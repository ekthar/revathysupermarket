...
const typeIcons: Record<string, { icon: React.ElementType; bg: string; color: string }> = {
  ...
  refund: { icon: Receipt, bg: "bg-emerald-50 dark:bg-emerald-950/30", color: "text-emerald-600" },
  ...
  if (notifications.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h2 className="text-[16px] font-bold text-slate-900 dark:text-white">No notifications</h2>
        <p className="text-[13px] text-slate-500 dark:text-slate-400 mt-1">Order updates and alerts will appear here</p>
      </div>
    );
  }
      ...
      {notifications.map((notif, i) => {
        ...
        return (
          <motion.div
            ...
            className={`rounded-2xl p-4 ${notif.read ? "bg-white dark:bg-slate-900" : "bg-primary/5 dark:bg-primary/10 border border-primary/10"} card-shadow`}
          >
            <div className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="text-[10px] text-slate-400 shrink-0">{timeAgo(notif.createdAt)}</span>
                </div>
                <p className="text-[12px] text-slate-500 dark:text-slate-400 mt-0.5 leading-relaxed">{notif.body}</p>
                {notif.orderId && (
                  <Link href="/dashboard" className="mt-2 inline-block text-[11px] font-semibold text-primary">
                    View Order →
                  </Link>
                )}
              </div>
