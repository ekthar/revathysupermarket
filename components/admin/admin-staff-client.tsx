  ...
  return (
    <div>
      ...
      <form onSubmit={addStaff} className="mt-5 grid gap-3 rounded-[1.75rem] border border-white/70 bg-card/95 p-4 shadow-soft dark:border-white/10 sm:grid-cols-2">
        ...
        <select value={form.role} onChange={(event) => setForm((current) => ({ ...current, role: event.target.value }))} className="h-11 rounded-2xl border border-border bg-background px-4 text-sm font-bold outline-none focus:ring-2 focus:ring-primary">
          {["OWNER", "MANAGER", "PACKING_STAFF", "DELIVERY_PARTNER", "STAFF"].map((role) => <option key={role} value={role}>{role}</option>)}
        </select>
          ...
          <article key={member.id} className="rounded-[1.5rem] border border-white/70 bg-card/95 p-4 shadow-soft dark:border-white/10">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">{member.email} · {member.phone ?? "No phone"} · {member.role}</p>
