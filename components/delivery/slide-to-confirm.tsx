 ...
 * Full-width track with draggable thumb. Must drag >85% to trigger.
 * Haptic feedback on complete. Spring-animated reset on incomplete.
 */
export function SlideToConfirm({ label = "Slide to confirm", onConfirm, disabled = false }: SlideToConfirmProps) {
  ...
  return (
    ...
    >
      ...
      <motion.span style={{ opacity: labelOpacity }} className={`absolute inset-0 flex items-center justify-center text-sm font-black ${disabled ? "text-slate-400" : "text-white/70 dark:text-slate-900/70"}`}>
        {label} &rarr;
      </motion.span>
      <motion.div style={{ scale: checkScale }} className="absolute right-4 flex h-8 w-8 items-center justify-center rounded-full bg-emerald-500">
      ...
      <motion.div
        ...
        className={`relative z-10 ml-1 flex h-14 w-14 cursor-grab items-center justify-center rounded-xl shadow-lg active:cursor-grabbing ${disabled ? "bg-slate-300 dark:bg-slate-600" : "bg-emerald-500"}`}
      >
