...
export function HomeHero({ title, subtitle, href, image, deliveryRadiusKm, gstEnabled }: HomeHeroProps) {
  ...
  return (
    <section className="relative isolate overflow-hidden rounded-b-[2rem] bg-slate-950 sm:rounded-b-[3rem]">
      <div className="absolute inset-0 bg-cover bg-center will-change-auto" style={{ backgroundImage: `url("${image}")` }}>
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(8,15,14,0.48),rgba(8,13,25,0.88)),linear-gradient(110deg,rgba(7,95,66,0.86),rgba(15,23,42,0.12)_58%,rgba(167,209,41,0.18))]" />
      </div>

      <div className="relative mx-auto flex min-h-[590px] max-w-7xl items-end px-4 pb-24 pt-16 sm:min-h-[640px] sm:px-6 sm:py-20 lg:px-8">
        <div className="max-w-3xl text-white">
          ...
          <motion.div initial={initial} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14, duration: 0.35 }} className="mt-7 grid grid-cols-2 gap-2 rounded-[1.5rem] border border-white/25 bg-slate-950/40 p-2 sm:grid-cols-4">
            {[
