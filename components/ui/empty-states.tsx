...
export function EmptyOrdersState() {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
      ...
      <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mt-1.5 text-[13px] text-slate-500 dark:text-slate-400 max-w-xs">
        Your order history will appear here once you place your first order.
      </motion.p>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
        <Link href="/products" className="mt-5 inline-flex h-10 px-6 items-center justify-center rounded-full bg-primary text-white text-[13px] font-bold press">
          Start Shopping
        </Link>
      </motion.div>
    </div>
